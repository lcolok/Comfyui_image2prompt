...
class VisionEncoder(nn.Module):
    def __init__(self, model_path: str = "model",device="cpu",dtype=torch.float32) -> None:
        super().__init__()
        self.model = torch.jit.load(f"{model_path}/vision.pt").to(device=device, dtype=dtype)
        self.device = device
        self.dtype = dtype
    ...
    def __call__(self, image: Image) -> torch.Tensor:
        with torch.no_grad():
            ...
            image_vec = image_vec.to(self.device, dtype=self.dtype)
...
class MoondreamModel():
    ...
    def load_model(cls, device="cpu", dtype=torch.float32):
        ...
        return model_path, vision_encoder, text_model


    def cached_vision_encoder(self, image):
        # Calculate checksum of the image
        np_image = image.numpy() if isinstance(image, torch.Tensor) else image
        image_hash = hashlib.sha256(np_image.tobytes()).hexdigest()
        ...
        if os.path.exists(cache_path):
            ...
            image_vec = self.vision_encoder(image).to("cpu", dtype=torch.float16)
